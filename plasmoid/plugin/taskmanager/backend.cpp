    ...
    , m_activityManagerPluginsSettingsWatcher(KConfigWatcher::create(m_activityManagerPluginsSettings.sharedConfig()))
{
    ...
    //! Track the KWin WindowView effect the same way the Plasma 6 applet's
    //! effectWatcher does, so windowViewAvailable reflects reality.
    const QString windowViewService = QStringLiteral("org.kde.KWin.Effect.WindowView1");

    auto *effectWatcher = new QDBusServiceWatcher(windowViewService,
                                                  QDBusConnection::sessionBus(),
                                                  QDBusServiceWatcher::WatchForRegistration
                                                      | QDBusServiceWatcher::WatchForUnregistration,
                                                  this);

    connect(effectWatcher, &QDBusServiceWatcher::serviceRegistered, this, [this]() {
        m_windowViewAvailable = true;
        Q_EMIT windowViewAvailableChanged();
    });
    connect(effectWatcher, &QDBusServiceWatcher::serviceUnregistered, this, [this]() {
        m_windowViewAvailable = false;
        Q_EMIT windowViewAvailableChanged();
    });

    m_windowViewAvailable = QDBusConnection::sessionBus().interface()
        && QDBusConnection::sessionBus().interface()->isServiceRegistered(windowViewService).value();
}
...
//! ---- Restored / re-implemented for Latte ---------------------------------
//!
//! Plasma 6 removed these from Backend. The pure-Qt ones are taken from the
//! Plasma 5 Backend; the window ones are re-implemented over the same KWin
//! D-Bus interfaces the Plasma 6 applet now calls from QML, so that Latte's
//! QML can keep calling backend.<x>() unchanged.
...
{
    if (m_highlightWindows == highlight) {
        return;
    }
...
QStringList Backend::windowIdsFrom(const QVariant &winIds) const
{
    QStringList ids;
...
void Backend::windowsHovered(const QVariant &winIds, bool hovered)
{
    ...
    msg << (hovered ? windowIdsFrom(winIds) : QStringList());
...
void Backend::activateWindowView(const QVariant &winIds)
{
    ...
    msg << windowIdsFrom(winIds);
    QDBusConnection::sessionBus().asyncCall(msg);
}

QVariantMap Backend::generateMimeData(const QString &mimeType, const QVariant &mimeData, const QUrl &url) const
{
